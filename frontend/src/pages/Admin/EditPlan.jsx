...
const EditPlan = () => {
  ...
  const handleDayToggle = (day) => {
    if (activeDays.includes(day)) {
      ...
      setSchedule((prev) => prev.filter((d) => d.dayName !== day));
    } else {
      ...
      setSchedule((prev) => [
        ...prev,
        {
          ...
          diets: [{ dietId: [], mealTime: "", mealType: "" }],
        },
      ]);
    }
  };
      ...
      exercises: day.exercises.map((ex) => ({
        ...
        exerciseId:
          typeof ex.exerciseId === "object"
            ? ex.exerciseId._id || ex.exerciseId
            : ex.exerciseId,
      })),
      diets: day.diets.map((dt) => ({
        ...dt,
        dietId: Array.isArray(dt.dietId)
          ? dt.dietId.map((d) => (typeof d === "object" ? d._id || d : d))
          : [],
      })),
    }));
    ...
    <div className="min-h-screen  text-white px-4 sm:px-8 md:px-12 py-10">
      <div className="max-w-5xl mx-auto bg-[#1a241a] p-6 sm:p-8 rounded-2xl shadow-lg">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-5">
          ...
          <div>
            <label className="block font-semibold mb-3">Active Days</label>
          ...
          {schedule.map((day) => (
            ...
            >
              ...
              {/* <button
                type="button"
                onClick={() => handleDeleteExercise(day.dayName, i)}
                className="bg-red-600 hover:bg-red-700 text-white text-xs px-2 py-1 rounded ml-2"
              >
                Delete
              </button> */}
              ...
              {day.diets.map((dt, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    >
                      <span className="capitalize text-sm">
                        {(typeof dt.dietId === "object"
                          ? dt.dietId.items
                          : diets.find((d) => d._id === dt.dietId)?.items) ||
                          `Diet ${i + 1}`}
                      </span>
